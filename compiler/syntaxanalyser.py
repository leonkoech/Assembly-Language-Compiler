...
print('Syntax Analyser is running...............................................')
...
tk_scanner,tk_new,tk_system,tk_out,tk_in,tk_Adas,tk_For,tk_return,tk_unknown,tk_and_sym= range(67)
...
Tokens = [
    ...
    ['system'          , False, False, False, -1, -1        ],
    ['out'             , False, False, False, -1, -1        ],
    ['in'              , False, False, False, -1, -1        ],
    ['+='              , False, False, False, -1, nd_Adas   ],
    ['for'             , False, False, False, -1, nd_For    ],
    ['return'          , False, False, False, -1, nd_Return ],
    ['?'               , False, False, False, -1, -1        ],
    ['&'               , False, False, False, -1, -1        ]
    ]
...
all_syms = {"End_of_input"   : tk_EOI,     "Op_multiply"    : tk_Mul,
            ...
            "Annotation_sym" : tk_annotation,"Keyword_println": tk_println,
            ...
            'Type_new'       : tk_new,'tk_System'           :tk_system,
            'tk_out'         : tk_out,'tk_in'               :tk_in,
            'tk_addassign'   : tk_Adas,'Keyword_For'        :tk_For,
            'Keyword_return' : tk_return,'Question_symbol'  :tk_unknown,
            'And_Symbol'     : tk_and_sym,
            }
...
def expr(p):
    ...
    if tok == tk_Lparen:
        ...
        gettok()
        if tok==tk_Lparen:
            expect("leftparenth",tk_Lparen)
            while tok!=tk_Rparen and tok != tk_EOI:
                x = make_leaf(nd_Ident, tok_other)
                gettok()
            expect("Lparen",tk_Rparen)
        if tok==tk_period:
            gettok()
        if tok==tk_Semi:
        ...
        if tok==tk_Adas:
            gettok()
            x = make_node(nd_Adas, expr(Tokens[tk_Adas][TK_PRECEDENCE]))
        # for for loops
        # i=2;i<count;++i
        if tok==tk_Assign:
            gettok()
            if tok==tk_Integer:
                ...
                if tok==tk_Semi:
                    ...
                    if tok==tk_Ident:
                        gettok()
                        # for i<x or i<=x or i>x or i>=x
                        if tok==tk_Lss or tok==tk_Gtr or tk_Leq or tk_Geq:
                            gettok()
                            # for i<ident or i<int
                            if tok==tk_Ident or tok==tk_Integer:
                                gettok()
                                if tok==tk_Semi: 
                                    gettok()
                                    # for ident,add,add(i++) or i-- or ++i or --i
                                    if tok==tk_Add or tok==tk_Ident or tok==tk_Sub:
                                        gettok()
                                        if tok==tk_Add or tok==tk_Sub:
                                            gettok()
                                            if tok==tk_Add or tok==tk_Ident or tok==tk_Sub:
                                                print('yahoo')
                ...
                # for example ident,assign,lrparen,ident,rparen i=(i)
                gettok()
                while tok!=tk_Rparen:
                    x = make_leaf(nd_Ident, tok_other)
                    gettok()
        ...
        if tok==tk_Comma:
            ...
            if tok==tk_Integer:
                ...
                x = make_leaf(nd_String, tok_other)
            elif tok==tk_Ident:
                x = make_leaf(nd_Ident, tok_other)
            elif tok == tk_and_sym:
                gettok()
            ...
            gettok()
            x = make_node(nd_Adas, expr(Tokens[tk_Not][TK_PRECEDENCE]))
 
    elif tok==tk_annotation:
        gettok()
        if tok==tk_Ident:
            gettok()
            if tok==tk_Lparen:
        ...
        if tok==tk_Print:
            ...
            if tok == tk_Rparen:
                gettok()
    elif tok==tk_IntegerType or tok==tk_CharacterType or tok==tk_ShortType or tok==tk_LongType or tok==tk_DoubleType or \
    tok==tk_StringType or tok==tk_BooleanType or tok==tk_ByteType or tok==tk_ArrayType or tok==tk_Public or tok==tk_Private or \
    tok==tk_Static or tok==tk_void:
        gettok()
    elif tok==tk_Rparen:
       pass
    elif tok==tk_Rbrace:
...
def paren_expr():
    ...
    return node
def parent_expr():
    expect("parent_expr", tk_Lparen)
    node = expr(0)
    expect("parent_expr", tk_Rparen)
    return node
#***
def stmt():
    t = None
    ...
    if tok == tk_If:
       ...
       gettok()
        ...
        gettok()
        while tok!=tk_Semi and tok !=tk_EOI:
            x = make_leaf(nd_Return, tok_other)
            gettok()
    elif tok==tk_annotation:
        gettok()
        if tok==tk_Ident:
        ...
        while True:
            ...
            if tok == tk_Integer:
                e = make_node(nd_Prti, make_leaf(nd_String, tok_other))
                gettok()
            if tok == tk_String:
        ...
        if tok==tk_Mul:
            expect("mul", tk_Mul)
            e = expr(0)
            t = make_node(nd_Mul, v, e)
        elif tok==tk_Adas:
            e=expr(0)
            t = make_node(nd_Adas,v,e)
        elif tok==tk_Comma:
            gettok()
            if tok==tk_Integer:
                gettok()
        ...
        e = parent_expr()
        s = stmt()
        t = make_node(nd_For, e, s)
    elif tok == tk_Lparen:
        gettok()
        while tok != tk_Rparen and tok != tk_EOI:
            # gettok()
            t = make_node(nd_String, t, stmt())
            gettok()
            while True:
                if tok==tk_Adas:
                    t = make_node(nd_Adas, t, stmt())
        expect("Lparen", tk_Rparen)
    elif tok == tk_Lbrace:
        gettok()
    ...
    # if start is a preprocessor directive
        ...
        # gettok()
        pass
    elif tok==tk_IntegerType or tok==tk_CharacterType or tok==tk_ShortType or tok==tk_LongType or tok==tk_DoubleType or tok==tk_StringType or tok==tk_BooleanType or tok==tk_ByteType or tok==tk_ArrayType or tok==tk_Public or tok==tk_Private or tok==tk_Static or tok==tk_void:
        gettok()
    elif tok==tk_Integer:
        gettok()
        if tok==tk_Comma:
